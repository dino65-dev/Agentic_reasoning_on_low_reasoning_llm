...
from agentic_engine.reasoning_engine.reasoning_engine import ReasoningEngine
from agentic_engine.selection_module import SelectionModule
...
async def process_single_problem_async(
    ...
    # Process
    engine = ReasoningEngine(tools=["calculator", "llm", "symbolic", "regex"])
    selector = SelectionModule()
    
    # Run reasoning in thread pool to avoid blocking
    loop = asyncio.get_event_loop()
    trace_list, _ = await loop.run_in_executor(
        None,
        engine.reason_over_options,
        problem_statement,
        answer_options
    )
